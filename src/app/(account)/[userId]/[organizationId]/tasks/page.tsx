...
export default async function Tasks({ params }: { params: any }) {
  ...
  const organizedTasks: TasksInterface[] = fetchOrganizationData.tasks.map(
    ({ _id, taskName, roles, priority, deadline, status }: TasksInterface) => ({
      _id: _id.toString(),
      taskName,
      roles,
      priority,
