...
export default function ShoutsSection({
  ...
  useEffect(() => {
    const filter = shouts.filter((shout) => {
      ...
      const roleMatch =
        pageState.role === "All" ? true : Object.values(shout.relevantRoles[0]).includes(pageState.role);
      const matchesDate =
    ...
    <>
      <div className={styles.top_area}>
        ...
        <div className={styles.categories}>
          ...
          <select name="role" id="role-selection" onChange={updateChange} value={pageState.role}>
            <option>All</option>
            <option>Owner</option>
            <option>Guest</option>
          </select>
        ...
        {filteredShouts.map((shout: ShoutsInterface) => (
          <div key={shout._id} className={styles.shout}>
            ...
            <div className={styles.roles_to_address}>
              ...
              {shout.relevantRoles.map((role: string) => (
                <p key={role}>{role}</p>
              ))}
            </div>
            <details>
