    ...
    { _id: userId },
    {
      $push: {
        organizations: {
          ...
          name: getOrganizationFromDB.organizationName,
          joined: dateString,
          role: "Owner",
        },
      },
...
export async function leaveOrganization(userId: string, organizationName: string) {
  await dbConnect();
  await User.findOneAndUpdate({ _id: userId }, { $pull: { organizations: { name: organizationName } } }, { new: true });
  await updateSession(userId);
}
...
export async function updateUserData(originalUserData: UpdateUser, userFormInput: UpdateUser) {
  ...
  const confirmedEmail = emailRegex.test(userFormInput.email) ? userFormInput.email : originalUserData.email;
  const confirmedPassword = passwordRegex.test(userFormInput.password as string) ? true : false;
...
export async function inviteNewMember(userEmail: string, organizationId: string, userId?: string) {
  ...
  const checkIfUserIsAlreadyInvited =
    getUserFromDB.invites.some((invite: any) => invite._id.toString() === getOrganizationFromDB._id.toString()) ||
    false;
  const checkIfUserInOrganization =
    getOrganizationFromDB.members.some((member: any) => member.memberEmail === userEmail) || false;
...
export async function acceptInvite(userId: string, inviteId: string, inviteName: string) {
  await dbConnect();
  await User.findOneAndUpdate({ _id: userId }, { $pull: { invites: { _id: inviteId } } }, { new: true });
  const dateobj = new Date();
  const dateString = `${dateobj.getMonth() + 1}/${dateobj.getDate()}/${dateobj.getFullYear()}`;
  await User.findOneAndUpdate(
    { _id: userId },
...
export async function declineInvite(userId: string, inviteName: string) {
  await dbConnect();
  await User.findOneAndUpdate({ _id: userId }, { $pull: { invites: { name: inviteName } } }, { new: true });
  await updateSession(userId);
}
...
export async function updateTasksList(
  ...
  updatedTasks: TasksInterface[],
  operationType: string,
  tasksToRemove: string[] | undefined,
  userId: string
) {
  await dbConnect();
  for (const updatedTask of updatedTasks) {
    await Organization.updateOne(
      ...
      {
        arrayFilters: [{ "task._id": updatedTask._id }],
      }
    );
  }
