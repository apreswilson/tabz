...
export async function createUser(prevState: { message: string }, formData: FormData) {
  ...
  if (!parse.success) {
    return {
      message:
        "First and last name must be 2 characters. Email must be a valid email address. Password must be 8 characters and include 1 letter, number, and special character.",
    };
  }
